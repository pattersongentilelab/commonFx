...
for x = 1:nVar 
    CI95 = ExpCalc95fromSE(table2array(mdl.Coefficients(x,1)),table2array(mdl.Coefficients(x,2)));
...
plot([1 1],[varNum(1) varNum(end)],'--k')
title('Model')
ax = gca; ax.Box = 'on'; ax.YTick = varNum(1:end-1); ax.YTickLabels = flipud(VarNames(2:end)); ax.YLim = [0 length(varNum)+1]; ax.XLim = [min(tbl.low95)*0.9 max(tbl.hi95)*1.1]; ax.XScale = 'log';
